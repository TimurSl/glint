...
bool FFmpegEncoder::open() {
    ...
    auto setupAudio = [&](AudioEncoderState &state) {
        ...
        state.frame_samples = state.ctx->frame_size > 0 ? state.ctx->frame_size : 1024;
...
bool FFmpegEncoder::prepareVideoFrame(const uint8_t *rgba, int w, int h, int stride, uint64_t pts_ms) {
    ...
    if (w != video_width_ || h != video_height_) {
        Logger::instance().warn("FFmpegEncoder: frame size mismatch");
    }
    const uint8_t *src[4] = {rgba, nullptr, nullptr, nullptr};
    int srcStride[4] = {stride, 0, 0, 0};
    if (sws_scale(scaler_, src, srcStride, 0, h, video_frame_->data, video_frame_->linesize) <= 0) {
        return false;
    }
    video_frame_->pts = static_cast<int64_t>(pts_ms);
    return true;
}
...
bool FFmpegEncoder::encodeAudioSamples(AudioEncoderState &state, const float *interleaved, int samples, int sr, int ch,
                                       uint64_t pts_ms, EncodedStreamType type, std::vector<EncodedPacket> &out) {
    if (!state.enabled || !state.ctx || !state.frame) return false;
    ...
    state.frame->nb_samples = converted;
    state.frame->pts = av_rescale_q(static_cast<int64_t>(pts_ms), AVRational{1, 1000}, state.ctx->time_base);
    return encodeFrame(state.ctx, state.frame, type, out);
}
...
bool FFmpegEncoder::pushAudioF32(const float *interleaved, int samples, int sr, int ch, uint64_t pts_ms, bool mic) {
    ...
    if ((state.input_sample_rate > 0 && sr != state.input_sample_rate) ||
        (state.input_channels > 0 && ch != state.input_channels)) {
        Logger::instance().warn("FFmpegEncoder: audio format mismatch");
    }
